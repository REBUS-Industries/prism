...
`docker compose pull` (see `.github/actions/ghcr-login-vm`). Use `GHCR_PULL_TOKEN`
for polyrepo packages. `redeploy-prod` falls back to local images or a VM-side git
build of `prism-fixtures-service` (via `ORBIT_DEPLOY_PAT`) when pull fails.
