  ...
  return {
    ...
    info: {
      ...
      description: [
        ...
        '',
        'Set `selectLayers=true` on `POST /convert/async` when you want to choose',
        'which Rhino layers to include before the conversion runs. The job',
        'lifecycle becomes:',
        '',
        '```',
        ...
        '```',
        '',
        'Stream `GET /jobs/{id}/stream` to receive each transition as an SSE',
        '`update` event. When `status` becomes `awaiting_selection`, the same',
        'event\'s `data` payload includes the `layers` tree inline so the',
        'follow-up `GET /jobs/{id}/layers` is optional. The flow is also',
        'available verbatim via the convenience UI at `/convert`.',
        '',
        'Direct callers that already know the layer names can skip the polling',
        'phase by passing `includedLayers` (and optionally',
        '`includeLayerDescendants=true`) on the original `/convert/async` call.',
        '',
        '## Webhooks',
        '',
