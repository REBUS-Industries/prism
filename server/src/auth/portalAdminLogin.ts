 ...
 *
 * Validates the OAuth code via prism-permissions, checks PORTAL_ADMIN_EMAILS,
 * then issues the same signed `prism_admin` cookie as password login.
 */
import { eq } from 'drizzle-orm';
import type { FastifyReply, FastifyRequest } from 'fastify';
import { db } from '../db/client.js';
import { adminUsers } from '../db/schema.js';
import { setAdminSession } from './adminSession.js';
...
async function findAdminUserForPortalEmail(email: string) {
  ...
  if (provisioned.allowed) {
    const bindUser = (
      provisioned.prismAdminUsername ??
      (await getSetting('portal_admin_username')) ??
      process.env.PORTAL_ADMIN_USERNAME ??
      process.env.ADMIN_USERNAME ??
      'admin'
    ).trim();
    const bound = await db.select().from(adminUsers).where(eq(adminUsers.username, bindUser)).limit(1);
  ...
  const bound = await db.select().from(adminUsers).where(eq(adminUsers.username, bindUser)).limit(1);
  return bound[0]?.isActive ? bound[0] : null;
}
