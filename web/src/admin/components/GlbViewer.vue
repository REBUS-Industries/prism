...
type EnvPreset = 'blank' | 'studio' | 'outdoor' | 'workshop' | 'night';
type ToneMap = 'aces' | 'agx' | 'neutral' | 'none';
type BgMode = 'none' | 'env' | 'grey';

const HDRI_PRESETS: Record<Exclude<EnvPreset, 'blank'>, { label: string; file: string }> = {
...
const ENV_OPTIONS: Array<{ id: EnvPreset; label: string }> = [
...
function applyAlphaAndSide(
  ...
  const nextSide: THREE.Side = activeShape.value === 'plane'
    ? THREE.DoubleSide
    : (p.doubleSided ? THREE.DoubleSide : THREE.FrontSide);
...
function buildBlankEnvironment(renderer: THREE.WebGLRenderer): THREE.WebGLRenderTarget {
  const pmrem = new THREE.PMREMGenerator(renderer);
  const greyScene = new THREE.Scene();
  greyScene.background = new THREE.Color(0x808080);
  greyScene.add(new THREE.AmbientLight(0xffffff, 1.0));
  const rt = pmrem.fromScene(greyScene, 0.04);
  pmrem.dispose();
  return rt;
}
...
async function loadEnvironment(): Promise<void> {
  ...
  try {
    let newRT: THREE.WebGLRenderTarget;

    if (activeEnv.value === 'blank') {
      newRT = buildBlankEnvironment(renderer);
    } else {
      try {
        const preset = HDRI_PRESETS[activeEnv.value];
        const tex = await rgbeLoader.loadAsync(preset.file);
        const pmrem = new THREE.PMREMGenerator(renderer);
        newRT = pmrem.fromEquirectangular(tex);
        pmrem.dispose();
        tex.dispose();
      } catch (e) {
        ...
        newRT = pmrem.fromScene(room, 0.04);
        room.dispose();
        pmrem.dispose();
      }
    }
...
function resetEnvRotation(): void {
  envRotation.value = 0;
}

function onDocumentPointerDown(ev: PointerEvent): void {
  if (!viewOpen.value) return;
  const el = viewMenuRef.value;
  if (el && !el.contains(ev.target as Node)) viewOpen.value = false;
}

function onDocumentKeyDown(ev: KeyboardEvent): void {
  if (ev.key === 'Escape') viewOpen.value = false;
}
...
<template>
  <div class="glb-viewer">
    <div ref="viewMenuRef" class="view-menu-wrap">
      <button
        type="button"
        class="view-trigger"
        :class="{ open: viewOpen }"
        ...
        @click.stop="viewOpen = !viewOpen"
      >
        <Icon name="tune" :size="16" />
        <span>View</span>
      </button>

      <div v-if="viewOpen" class="view-dropdown nodrag nopan" @click.stop>
        <section class="view-section">
          <span class="view-section-label">Preview shape</span>
        ...
        <section class="view-section">
          ...
          <select
            ...
            :disabled="envLoading"
          >
            <option v-for="opt in ENV_OPTIONS" :key="opt.id" :value="opt.id">
              {{ envLoading && activeEnv === opt.id ? '⟳ ' : '' }}{{ opt.label }}
            </option>
          </select>
        </section>

        <section class="view-section" title="Rotates the HDRI environment, not the mesh">
          <div class="view-row">
            <span class="view-section-label">Light rotation</span>
            <span class="view-readout">{{ envRotation }}°</span>
            <button
              v-if="envRotation !== 0"
              type="button"
              class="view-reset"
              title="Reset light rotation"
              @click="resetEnvRotation"
            >Reset</button>
          </div>
          <input
        ...
        <section class="view-section">
          <div class="view-row">
            <span class="view-section-label">Exposure</span>
            <span class="view-readout">{{ exposure.toFixed(1) }}</span>
            <button
              v-if="exposure !== 1.0"
              type="button"
              class="view-reset"
        ...
        <section class="view-section">
          <span class="view-section-label">Tone mapping</span>
          <div class="view-pills">
            <button
...
.view-reset:hover { color: var(--color-text); }
...
.view-select:disabled { opacity: 0.6; }
...
.view-pill:hover { color: var(--color-text); border-color: var(--color-border-strong); }
.view-pill.active {
