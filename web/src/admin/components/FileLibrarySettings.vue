    ...
    <div v-if="pickerOpen" class="picker-backdrop" @click.self="closePicker">
      <div class="picker">
        <header class="picker-head">
          ...
          <button @click="closePicker"><Icon name="close" :size="16" /></button>
        </header>

        <div class="crumbs">
          <button class="crumb" :disabled="browseLoading" @click="loadBrowse('')">share root</button>
...
.proj-table th, .proj-table td { padding: 10px 12px; text-align: left; border-bottom: 1px solid var(--color-border, #2a2a32); vertical-align: top; }
.proj-table th { background: var(--surface-2, #1a1a1f); }
...
.btn-link.danger { color: var(--color-danger, #e35d6a); }
...
.picker-head {
  ...
  padding: 14px 16px; border-bottom: 1px solid var(--color-border, #2a2a32);
}
.picker-head h3 { margin: 0 0 2px; font-size: 16px; }
.crumbs {
...
.crumb {
  ...
  padding: 2px 4px; text-decoration: underline; opacity: 0.85;
}
...
.dir-row {
  ...
  color: inherit; cursor: pointer; border-radius: 6px; font-size: 13px;
}
.dir-row:hover { background: var(--surface-hover, rgba(255, 255, 255, 0.04)); }
.picker-foot {
  display: flex; justify-content: flex-end; gap: 8px;
  padding: 12px 16px; border-top: 1px solid var(--color-border, #2a2a32);
}
</style>
