...
/**
 * One row in the admin layer-selection tree (recursive).
 *
 * Mirrors the public convert SPA's LayerRow but lives in the admin tree so
 * the two-phase `awaiting_selection` picker is self-contained here. Selection
 * is keyed on `fullPath ?? name` to match Rhino's `Layer.FullPath` (what the
 * server filters against — see server/src/api/jobs.ts POST /:id/layers).
 */
...
.expander.placeholder { cursor: default; }
.layer-label {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  font-size: 13px;
  /* Override the design-system's global uppercase/bold <label> styling so
     layer names render in their natural case (e.g. "Closed Extrusion"). */
  font-weight: 400;
  text-transform: none;
